...
public record SealedLetterContent(Filterable<String> title, String author, int generation, List<Filterable<Component>> pages, boolean resolved) {
        ...
        public static final Codec<SealedLetterContent> CODEC;
        public static final StreamCodec<RegistryFriendlyByteBuf, SealedLetterContent> STREAM_CODEC;
    ...
    public static boolean resolveForItem(final ItemStack itemStack, final ResolutionContext context, final HolderLookup.Provider registries) {
        SealedLetterContent content = (SealedLetterContent)itemStack.get(LOEDataComponents.SEALED_LETTER_CONTENT);
        ...
        public @Nullable SealedLetterContent resolve(final ResolutionContext context, final HolderLookup.Provider registries) {
            if (this.resolved) {
                return null;
            } else {
        ...
        static {
            ...
            CODEC = RecordCodecBuilder.create((i) -> i.group(Filterable.codec(Codec.string(0, 32)).fieldOf("title").forGetter(SealedLetterContent::title), Codec.STRING.fieldOf("author").forGetter(SealedLetterContent::author), ExtraCodecs.intRange(0, 3).optionalFieldOf("generation", 0).forGetter(SealedLetterContent::generation), PAGES_CODEC.optionalFieldOf("pages", List.of()).forGetter(SealedLetterContent::pages), Codec.BOOL.optionalFieldOf("resolved", false).forGetter(SealedLetterContent::resolved)).apply(i, SealedLetterContent::new));
            STREAM_CODEC = StreamCodec.composite(Filterable.streamCodec(ByteBufCodecs.stringUtf8(32)), SealedLetterContent::title, ByteBufCodecs.STRING_UTF8, SealedLetterContent::author, ByteBufCodecs.VAR_INT, SealedLetterContent::generation, Filterable.streamCodec(ComponentSerialization.STREAM_CODEC).apply(ByteBufCodecs.list()), SealedLetterContent::pages, ByteBufCodecs.BOOL, SealedLetterContent::resolved, SealedLetterContent::new);
        }
